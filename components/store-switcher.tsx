...
export default function StoreSwitcher({ className, items=[] } : StoreSwitcherProps)
{
    ...
    const onStoreSelect = function(store: { label: string, value: string})
    {
        setOpen(false);
                ...
                <Command>
                    <CommandList>
                        ...
                        <CommandGroup heading="Stores">
                            {formattedItems.map((store) => (<CommandItem key={store.value} onSelect={() => onStoreSelect(store)} className="text-sm">
                                <StoreIcon className="mr-2 h-4 w-4"/>
                                {store.label}
